...
# --- Torque curve points (kgfm → Nm conversion)
...
# --- Plot torque
            ...
            self.ax.plot(
                rpms, torques_nm, color=self.torque_color,
                label=f"{car_label} - Torque ({max_torque:.1f} Nm)"
            )
# --- Calculate & plot power
            powers = [(t * rpm) / 9549 for t, rpm in zip(torques_nm, rpms)]
            max_power = max(powers) if powers else 0
            self.ax2.plot(
                rpms, powers, linestyle="--", color=self.power_color,
...
# --- Metadata
            ...
            self.meta_box.append(f"==== {car_label} ====")
    ...
    def load_csv(self):
        ...
        if file:
            ...
            for _, row in self.data.iterrows():
                label = row.get("CarLabel", row.get("Label", "Unknown"))
